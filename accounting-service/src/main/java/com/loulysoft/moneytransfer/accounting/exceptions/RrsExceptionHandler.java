...
public class RrsExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleNotFoundException(RrsException exception) {
        // return buildErrorResponse(exception);
    ...
    //
    //    @RrsExceptionHandler(ValidationException.class)
    ...
    private RrsException mapToRrsException(Exception e, HttpStatus httpStatus) {
        ...
        log.error(
                "Exception occurred with responseId: {} errorCode: {} and message: '{}'",
                rrse.getId(),
    ...
    private ResponseEntity<ErrorResponse> buildErrorResponse(RrsException e) {
        String message =
                HttpStatus.INTERNAL_SERVER_ERROR == e.getHttpStatus() ? INTERNAL_SERVER_ERROR_MESSAGE : e.getMessage();
